...
class CCOMMONTOOLS_EXPORT CCommon
{
public:
    CCommon();
    ~CCommon();
    ...
    static std::wstring StrToUnicode(const std::string& str, CodeType code_type = CodeType::ANSI, UINT code_page = CP_ACP);

    //将Unicode编码的wstring字符串转换成string类型的字符串(如果至少有一个字符无法转换，则将char_cannot_convert置为true)
    static std::string UnicodeToStr(const std::wstring& wstr, bool& char_cannot_convert, CodeType code_type = CodeType::ANSI, UINT code_page = CP_ACP);
    ...
    //将一个字符串分割成若干个字符（模板类型只能为std::string或std::wstring）
    //str: 原始字符串
    //div_ch: 用于分割的字符
    //result: 接收分割后的结果
    template<class T>
    ...
    //读取文件内容
    static bool GetFileContent(const wchar_t* file_path, std::string& contents_buff, bool binary = true);
    ...
    */
    ...
    //判断文件夹是否存在
    static bool FolderExist(LPCTSTR folder_path);
    ...
    //获取临时文件夹的路径
    static std::wstring GetTemplateDir();

    //获取Appdata/Local/TrafficMonitor的目录，如果不存在，则会自动创建
    static std::wstring GetAppDataConfigDir();
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    static std::wstring GetJsonValueSimple(const std::wstring& json_str, const std::wstring& name);

    //获取URL的内容
    static bool GetURL(const std::wstring& url, std::wstring& result, bool utf8 = false, const std::wstring& user_agent = std::wstring());
