...
{
	bool tagFound = false;
	int caret = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCURRENTPOS));
	...
	do
	{
		...
		styleAt = _pEditView->SendMessage(SCI_GETSTYLEAT, openFound.start);
	...
	if (openFound.success && styleAt != SCE_H_CDATA)
	{
		...
		do
		{
			...
			styleAt = _pEditView->SendMessage(SCI_GETSTYLEAT, closeFound.start);
		...
		if (!closeFound.success)
		{
			...
			int nextChar = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCHARAT, openFound.start + 1));
			...
			if ('/' == nextChar)
			{
				...
				int docLength = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETLENGTH));
				...
				std::string tagName;
				nextChar = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCHARAT, position));
			...
			/////////////////////////////////////////////////////////////////////////
				...
				if (tagName.size() != 0)
				{
					...
					if (-1 != closeAnglePosition)
					{
						...
						if (_pEditView->SendMessage(SCI_GETCHARAT, closeAnglePosition - 1) == '/')
						{
...
XmlMatchedTagsHighlighter::FindResult XmlMatchedTagsHighlighter::findOpenTag(const std::string& tagName, int start, int end)
{
	...
	do
	{
		...
		if (result.success)
		{
			nextChar = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCHARAT, result.end));
			styleAt = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETSTYLEAT, result.start));
			if (styleAt != SCE_H_CDATA && styleAt != SCE_H_DOUBLESTRING && styleAt != SCE_H_SINGLESTRING && styleAt != SCE_H_COMMENT)
			{
				...
				else if (isWhitespace(nextChar))
				{
					...
					if (-1 != closeAnglePosition && '/' != _pEditView->SendMessage(SCI_GETCHARAT, closeAnglePosition - 1))
					{
...
int XmlMatchedTagsHighlighter::findCloseAngle(int startPosition, int endPosition)
{
	...
	do
	{
		...
		{
			auto style = _pEditView->SendMessage(SCI_GETSTYLEAT, closeAngle.start);
...
XmlMatchedTagsHighlighter::FindResult XmlMatchedTagsHighlighter::findCloseTag(const std::string& tagName, int start, int end)
{
	...
	do
	{
		...
		{
			nextChar = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCHARAT, result.end));
			auto styleAt = _pEditView->SendMessage(SCI_GETSTYLEAT, result.start);
			...
			if (styleAt != SCE_H_CDATA && styleAt != SCE_H_SINGLESTRING && styleAt != SCE_H_DOUBLESTRING && styleAt != SCE_H_COMMENT) // If what we found was in CDATA section, it's not a valid tag.
			{
				...
				else if (isWhitespace(nextChar))  // Otherwise, if it's whitespace, then allow whitespace until a '>' - any other character is invalid.
				{
					...
					{
						++whitespacePoint;
						nextChar = static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCHARAT, whitespacePoint));
...
XmlMatchedTagsHighlighter::FindResult XmlMatchedTagsHighlighter::findText(const char *text, int start, int end, int flags)
{
	...
	int result = static_cast<int32_t>(_pEditView->SendMessage(SCI_FINDTEXT, flags, reinterpret_cast<LPARAM>(&search)));
	if (-1 == result)
	{
...
void XmlMatchedTagsHighlighter::tagMatch(bool doHiliteAttr) 
{
    ...
    _pEditView->ClearAllMark(CScintillaEditView::MarkStyle::HTML_MARKS);
	...
	//	const int caret = 1 + static_cast<int32_t>(_pEditView->SendMessage(SCI_GETCURRENTPOS)); // +1 to deal with the case when the caret is between the angle and the question mark in "<?" (or between '<' and '%').
	...
	auto originalStartPos = _pEditView->SendMessage(SCI_GETTARGETSTART);
	auto originalEndPos = _pEditView->SendMessage(SCI_GETTARGETEND);
	auto originalSearchFlags = _pEditView->SendMessage(SCI_GETSEARCHFLAGS);
	...
	{
		//_pEditView->SendMessage(SCI_SETINDICATORCURRENT, SCE_UNIVERSAL_TAGMATCH);
		int openTagTailLen = 2;

		// Colourising the close tag firstly
		if ((xmlTags.tagCloseStart != -1) && (xmlTags.tagCloseEnd != -1))
		{
			_pEditView->SetMark(CScintillaEditView::MarkStyle::HTML_MARKS,  xmlTags.tagCloseStart, xmlTags.tagCloseEnd - xmlTags.tagCloseStart);
			// tag close is present, so it's not single tag
			openTagTailLen = 1;
		}

		// Colourising the open tag
		_pEditView->SetMark(CScintillaEditView::MarkStyle::HTML_MARKS,  xmlTags.tagOpenStart, xmlTags.tagNameEnd - xmlTags.tagOpenStart);
		_pEditView->SetMark(CScintillaEditView::MarkStyle::HTML_MARKS,  xmlTags.tagOpenEnd - openTagTailLen, openTagTailLen);
        ...
        if (doHiliteAttr)
		{
			...
			//_pEditView->SendMessage(SCI_SETINDICATORCURRENT,  SCE_UNIVERSAL_TAGATTR);
			//for (size_t i = 0, len = attributes.size(); i < len ; ++i)
			//{
			//	_pEditView->SendMessage(SCI_INDICATORFILLRANGE,  attributes[i].first, attributes[i].second - attributes[i].first);
			//}
        }
		...
		//{
		//	int columnAtCaret  = int(_pEditView->SendMessage(SCI_GETCOLUMN, xmlTags.tagOpenStart));
		//	int columnOpposite = int(_pEditView->SendMessage(SCI_GETCOLUMN, xmlTags.tagCloseStart));

		//	int lineAtCaret  = int(_pEditView->SendMessage(SCI_LINEFROMPOSITION, xmlTags.tagOpenStart));
		//	int lineOpposite = int(_pEditView->SendMessage(SCI_LINEFROMPOSITION, xmlTags.tagCloseStart));

		//	if (xmlTags.tagCloseStart != -1 && lineAtCaret != lineOpposite)
		//	{
		//		_pEditView->SendMessage(SCI_BRACEHIGHLIGHT, xmlTags.tagOpenStart, xmlTags.tagCloseEnd-1);
		//		_pEditView->SendMessage(SCI_SETHIGHLIGHTGUIDE, (columnAtCaret < columnOpposite)?columnAtCaret:columnOpposite);
		//	}
		//}
	}

	// restore the original targets and search options to avoid the conflit with search/replace function
	_pEditView->SendMessage(SCI_SETTARGETRANGE, originalStartPos, originalEndPos);
	_pEditView->SendMessage(SCI_SETSEARCHFLAGS, originalSearchFlags);
}
